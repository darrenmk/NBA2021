...
cdfPlot2 <- ggplot(cdfData, aes(x=timecourse)) +
  ...
  annotate("text",x=100,y=0.9,label=paste("VA:",round(grandVA[2],1),"ms",sep=" "),size=4) +
...
cdfPlot4 <- ggplot(cdfData, aes(x=timecourse)) +
  ...
  annotate("text",x=100,y=0.99,label="SOA: 50 ms",size=4,fontface="bold") +
  annotate("text",x=100,y=0.9,label=paste("VA:",round(grandVA[4],1),"ms",sep=" "),size=4) +
...
# Calculating mean and standard error of RT, MR, VA, and CRE
...
grandVA <- tapply(data$va,data$soa,mean,na.rm=1)
grandVAse <- tapply(data$va,data$soa,sd,na.rm=1)/sqrt(n)
grandME <- tapply(data$me,data$soa,mean,na.rm=1)
grandMEse <- tapply(data$me,data$soa,sd,na.rm=1)/sqrt(n)
...
grandData <- data.frame(grandRT,grandRTse,grandMR,grandMRse,grandMR_scaled,grandMRse_scaled,grandME,grandMEse,grandVA,grandVAse,grandSoa,grandCondition)
...
ageVA <- ggplot(dataSoa, aes(x=age, y=va, fill=soa)) +
  geom_smooth(aes(colour=soa),method=lm,se=0) +
  geom_point(colour="black",aes(shape=cond,fill=soa),size=4) +
  ...
  labs(x = "Age", y = "Violation Area (ms)", tag="C") +
  theme_classic(base_size=17) +
  theme(legend.position="none",panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), text = element_text(size=32), axis.text = element_text(colour = "black"))
print(ageVA)

## Figure 4d: Age & ME
ageME <- ggplot(dataSoa, aes(x=age, y=me, fill=soa)) +
  geom_smooth(aes(colour=soa),method=lm,se=0) +
  geom_point(colour="black",aes(shape=cond,fill=soa),size=4) +
  scale_colour_manual(values=c("#F0F4C3", "#C5E1A5", "#9CCC65", "#689F38", "#33691E")) +
  scale_shape_manual(values=21) +
...
ggarrange(ageRT, ageMR, ageVA, ageCRE, nrow = 2, ncol = 2)
